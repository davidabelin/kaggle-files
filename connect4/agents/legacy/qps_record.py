...
def my_agent(obs, config, N_STEPS=2, debug=False):
    ...
    if debug:
        print(f'\n##Data Recorder as Player {obs.mark}: Piece {1+obs.board.count(int(obs.mark))} ##') 
    ...
    piece = 1+obs.board.count(int(obs.mark))
    quick_pick = False
    scores = {}
    for col in valid_moves:
            ...
            for c in range(COLUMNS):
                if c == choice:
                    ...
                    if c in valid_moves:
                        ...
                        scores[c] = -1000 #instead of -np.inf! 
            if debug:
    ...
    if not quick_pick:   
        for col in range(COLUMNS):
            if col in valid_moves:
                ...
                scores[col] = -1000 #C*F opp_fours x my_fours instead of -np.inf!       
    ...
    new_row = pd.Series([int(choice),int(obs.mark),int(1+obs.board.count(int(obs.mark)))]+ \
                        [0]+[int(s) for s in scores.values()]+ \
                        [int(x) for x in obs.board],
