def my_agent(obs, config, START_TIME=None, N_STEPS=2, cutoff_time=None, debug=False):
    ...
    if list(obs.board).count(1) < 2: # or list(obs.board[0]).count(0) >= COLUMNS//2:
        N_STEPS =      1
    elif list(obs.board).count(1) <= ROWS*COLUMNS//(2*3):
        N_STEPS =      2 
    elif list(obs.board).count(1) <= 2*ROWS*COLUMNS//(2*3):
        N_STEPS =      3
    else:
        N_STEPS =      4
